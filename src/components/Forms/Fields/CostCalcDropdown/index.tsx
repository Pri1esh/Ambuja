import { CustomIcon, CostCalcFloatingInput } from '@components';
...
const CostCalcDropdown = (props: ISelectDropdownCustom) => {
  const {
    options = [],
    ...
    return dropdownOptions?.length > 0 ? (
      <ul>
        {dropdownOptions.map((item: ISelectDropdownOption, index: number) => (
          <li key={`${item?.label + index}`} className={`${selectedValue?.label === item?.label ? styles.selected : ''}`}>
            <Button
              onClick={(e) => {
                handleClick(e, item);
              }}
            ...
            {showSelectedTick && selectedValue?.label === item?.label && <CustomIcon iconName="tick" />}
          </li>
        ))}
      </ul>
    ) : (
...
export default CostCalcDropdown;
